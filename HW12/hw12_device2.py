...
def read(conn, mask):
    ...
    # 사용자로부터 'Register' 메세지를 수신한 경우
        ...
        for i in range(5):  # 5번 전송
            ...
            heart = str(random.randint(40, 140))           # 심박수
            step = str(random.randint(2000, 6000))         # 걸음수
            calorie_burn = str(random.randint(1000, 4000)) # 소모 칼로리
                
            msg = ': Device2 : Heartbeat = ' + heart + ', Steps = ' + step + ', Cal = ' + calorie_burn
            print(msg)  
            conn.send(msg.encode())
            time.sleep(5) # 보내고 5초 쉰다
        print('device2 complete')
        conn.send(b'device2 complete')
    

sock = socket()
sock.bind(('', port))
sock.listen(5)
print(str(port) + '(device2)에서 접속 대기중')
